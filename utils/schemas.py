...
class Monster(BaseModel):
    ...
    challenge_rating: str = Field(description="Challenge rating as string (e.g., '3', '1/2', '1/4')")
    proficiency_bonus: int = Field(ge=0, description="Proficiency bonus")
    saving_throws: List[str] = Field(default_factory=list, description="List of saving throw proficiencies")
    skills: List[str] = Field(default_factory=list, description="List of skill proficiencies")
    damage_resistances: List[str] = Field(default_factory=list, description="List of damage resistances")
    damage_immunities: List[str] = Field(default_factory=list, description="List of damage immunities")
    condition_immunities: List[str] = Field(default_factory=list, description="List of condition immunities")
    senses: str = Field(default="", description="Senses description")
    languages: str = Field(default="", description="Languages known")
    special_abilities: List[SpecialAbility] = Field(default_factory=list, description="List of special abilities")
    actions: List[MonsterAction] = Field(default_factory=list, description="List of actions/attacks")
    legendary_actions: List[LegendaryAction] = Field(default_factory=list, description="List of legendary actions")
    ...
    @field_validator('saving_throws', 'skills', 'damage_resistances', 'damage_immunities', 
                     'condition_immunities', 'special_abilities', 'actions', 'legendary_actions', mode='before')
    @classmethod
