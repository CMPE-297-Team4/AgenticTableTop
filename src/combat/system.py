...
class CombatEncounter:
    ...
    def start_combat(self):
        ...
        self.combat_log.append(
            f"Combat begins! Initiative order: {', '.join([c.name for c in self.initiative_order])}"
        )
...
def roll_attack_roll(
    attack_bonus: int, advantage: bool = False, disadvantage: bool = False
) -> Tuple[int, bool]:
    """
    Roll an attack roll.
...
def create_player_combatant(
    name: str, max_hp: int, armor_class: int, dexterity_modifier: int
) -> Combatant:
    """
...
def simulate_attack(
    ...
    if not attacker.is_conscious or not target.is_conscious:
        return {
            ...
            "damage": 0,
            "critical": False,
            "message": "Cannot attack while unconscious",
        }
    ...
    return {"hit": True, "damage": actual_damage, "critical": is_critical, "message": message}
...
def run_simple_encounter(
    ...
    # Combat log
    combat_log = []
    combat_log.append("=== COMBAT BEGINS ===")
    combat_log.append(
        f"Player: {player.name} (HP: {player.current_hp}/{player.max_hp}, AC: {player.armor_class})"
    )
    combat_log.append(
        f"Monster: {monster.name} (HP: {monster.current_hp}/{monster.max_hp}, AC: {monster.armor_class})"
    )
    combat_log.append(
        f"Initiative order: {', '.join([c.name for c in encounter.initiative_order])}"
    )
    combat_log.append("")
    ...
    while not encounter.is_combat_over() and round_count < max_rounds:
        ...
        if current_combatant == player:
            ...
            attack_result = simulate_attack(
                player, monster, player_attack_bonus, player_damage_dice, player_damage_bonus
            )
